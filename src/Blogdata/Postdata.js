    ...
    useEffect(() => {
        const fetchPostData = async () => {
            try {
                ...
                const response = await axios.get(`https://app.posthog.com/api/projects/95663/events/?event=$pageview&limit=1000`, {
                    headers: {
                ...
                responseJson.results.forEach(event => {
                    ...
                    if (match) {
                        ...
                        postMetrics[postId].visitors[distinctId].totalTimeSpent += timeOnPage;

                        // Check if this event is earlier than the recorded first seen date
                        const eventDate = new Date(event.timestamp);
                        if (eventDate < postMetrics[postId].visitors[distinctId].firstSeen) {
                ...
                const formattedPostMetrics = Object.entries(postMetrics).map(([postId, metrics]) => {
                    const visitorsArray = Object.entries(metrics.visitors).map(([visitorId, visitorMetrics]) => ({
                        visitorId: visitorId,
                        ...
                        firstSeenDate: visitorMetrics.firstSeen.toISOString().split('T')[0],
                            ...
                            <tbody>
                                {post.visitors.map(visitor => (
                                    <tr key={visitor.visitorId}>
                                        ...
                                        <td>{visitor.location}</td> {/* Display visitor location */}
                                    </tr>
                                ))}
                            </tbody>
                        </table>
